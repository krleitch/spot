...
for each row
begin
    update accounts set score = score + 1 where id = (SELECT account_id FROM posts WHERE id = new.post_id);
end$$
...
for each row
begin
    update accounts set score = score + 1 where id = (SELECT account_id FROM comments WHERE id = new.comment_id);
end$$
delimiter ;
