...
export class PostsStoreEffects {
  ...
  dislikePostEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((action) =>
      this.postsService.dislikePost(action.request).pipe(
        map(
          (response: DislikePostSuccess) =>
  ...
  likePostEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((action) =>
      this.postsService.likePost(action.request).pipe(
        map(
          (response: LikePostSuccess) =>
  ...
  unratedPostEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((action) =>
      this.postsService.unratedPost(action.request).pipe(
        map(
          (response: UnratedPostSuccess) =>
  ...
  deletePostEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((action) =>
      this.postsService.deletePost(action.request).pipe(
        map(
          (response: DeletePostSuccess) =>
  ...
  addPostEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((post: featureActions.AddRequestAction) =>
      this.postsService.addPost(post.request).pipe(
        map(
          (response: AddPostSuccess) =>
            new featureActions.AddSuccessAction(response)
        ),
        catchError((errorResponse) =>
  ...
  loadRequestEffect$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap((action) =>
      this.postsService.getPosts(action.request).pipe(
